...
const Page = ({api,img,name}) =>{
    return(
    <>
               <div className='flex justify-between'>
<h1 className='mb-4 ml-0 xl:ml-3 text-3xl font-bold tracking-tight text-center text-gray-900 '>All Results</h1>
<Link href={`/Pages/showAll/${name}`} className='mr-3'></Link>
            </div>
            <div className='ml-0 flex justify-center flex-wrap xl:grid grid-cols-2 xl:ml-20 xl:grid-cols-4 '>
            {api.results?.map(apis=>{
                return(
                    <Link  key={apis.id} href={`/Pages/results/${apis.id}`} className={Styles.CardImgs}>
                    <Image className={Styles.imgSet} width={1000} height={100} alt='...' src={`${img}/original/${apis.poster_path}`}/>
